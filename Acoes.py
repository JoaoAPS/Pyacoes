...
class Acoes:
	...
	Propriedades:
	`hist`: DataFrame com o histórico de todas movimentações já feitas. Possui as colunas:
		...
		- precoPorAcao: Preço de 1 ação na hora da execução da ordem, sem contar taxas
	...
	`carteira`: DataFrame com as ações atualmente em posse
		...
		- precoParaLucro: preço médio mínimo que a ação deve ser vendida para obtenção de lucro após contar as taxas, caso todas as ações sejam vendidas de uma vez
	A carteira sempre possui apenas uma linha por ação (código).
	Se uma ação que já está na carteira é comprada novamente por um preço diferente,
	a linha da ação correspondente é atualizada. O preço médio é recalculado com uma média
	ponderada de 2 ítems: as ações na carteira e as novas, e a quantidade é simplesmente
	acrescida o número de novas ações.
	Uma venda simplesmente descresce a quantidade atual, sem mudar o preço médio.
	O lucro deve ser calculado nessa hora.
	Podemos ver queo sistema de cálculo do preço médio não é trivial, e *a ordem importa*.
	Ele deve ser calculado desse jeito pois é o formato padrão usado pela receita federal.

	`histLucro`: DataFrame com o lucro/prejuízo obtido com cada venda de ação executada
		- codigo: código da ação
		- data: data de execução da ordem
		- qnt: quantidade de ações movimentadas na ordem
		- precoMedioCarteira: preço médio gasto por ação nas compras, contando taxas
	...
	`codigos`: DataFrame com a relação entre código de negociação e nome da empresa
	...
	- getAcoesEm(data)
	# Updates
	- addOrdem(tipo:str, codigo:str, data, qnt:int, precoPorAcao:float, taxas:float, shouldSort:bool=True)
	...
	- resumo()
	- movimentacaoMensal()
	- estimarLucro(codigo:str, precoPorAcao:float, qnt:int=-1, taxas:float=-1, verbose:bool=True)
	"""
	...
	def __init__(self, readFromFiles:bool=False):
		...
		readFromFiles : bool, optional
			Indica se o objeto deve usar os dados salvos nos arquivos (defualt=True)
		"""
		...
		if readFromFiles:
			...
			os.system('cp ' + str(dataDir / 'hist.csv') + ' ' + str(dataDir / 'backups/hist.csv'))
			...
			self.hist = pd.DataFrame(columns=["tipo", "codigo", "data", "qnt", "precoPorAcao", "taxas", "total", "precoMedio"])
			self.carteira = pd.DataFrame(columns=["codigo", "qnt", "precoMedio", "total", "precoParaLucro"]).set_index('codigo')
			self.histLucro = pd.DataFrame(columns=['codigo', 'data', 'qnt', 'precoMedioCarteira', 'precoMedioVenda', 'lucroMedio', 'lucro'])
			self.lucro = 0
		...
		if os.path.isfile(dataDir / 'codigos.csv'):
			...
			os.system('cp ' + str(dataDir / 'codigos.csv ') + str(dataDir / 'backups/codigos.csv'))
		else:
			self.codigos = pd.DataFrame(columns=['codigo', 'empresa']).set_index('codigo')
	...
	def getAcoesEm(self, data) -> Acoes:
		...
		for idx,ordem in self.hist[self.hist['data'] <= data].iterrows():
			result.addOrdem(ordem['tipo'], ordem['codigo'], ordem['data'], ordem['qnt'], ordem['precoPorAcao'], ordem['taxas'], shouldSort=False)
	...
	def addOrdem(self, tipo:str, codigo:str, data, qnt:int, precoPorAcao:float, taxas:float, shouldSort:bool=True):
		...
		shouldSort : bool, optional
			Indice se as propriedades do objeto devem ser ordenadas ao final (default=True)
		"""
		...
		if tipo == 'venda':
			...
			if qnt > self.carteira.at[codigo,'qnt']:
				print("Erro! Há uma ordem de venda de %d ações %s, mas na carteira há apenas %d dessas ações!"
					%(qnt, codigo, self.carteira.at[codigo,'qnt']))
				return
		#----------------------------
		
		
		total = qnt * precoPorAcao + taxas  if  tipo == 'compra'  else  qnt * precoPorAcao - taxas
		precoMedio = total / qnt
		ordem = pd.Series({'tipo':tipo, 'codigo':codigo, 'data':data, 'qnt':qnt, 'precoPorAcao':precoPorAcao, 'taxas':taxas, 'total':total, 'precoMedio':precoMedio})
		...
		if ordem['tipo'] == 'compra':
			#Se é a primeira vez que compramos essa ação, simplesmente adiciona à carteira
			if codigo not in self.carteira.index:
				self.carteira.loc[codigo] = ordem[['qnt', 'precoMedio', 'total']].copy()
			...
			#Se já existe essa ação na carteira
				...
				self.carteira.at[codigo,'precoMedio'] = ( (existingRow['qnt'] * existingRow['precoMedio']) + (ordem['qnt'] * ordem['precoMedio']) ) / (existingRow['qnt'] + ordem['qnt'])
				
				#Atualiza quantidades
				self.carteira.at[codigo,'qnt'] = existingRow['qnt'] + ordem['qnt']
				self.carteira.at[codigo,'total'] += ordem['total']
				
			#Recalcula preço para lucro
			self.carteira.at[codigo,'precoParaLucro'] = (self.carteira.at[codigo,'total'] + self.taxaMedia) / self.carteira.at[codigo,'qnt']
			
		
		if ordem['tipo'] == 'venda':
			#Calcula lucro da venda
			lucroMedio = ordem['precoMedio'] - self.carteira.at[codigo,'precoMedio']
			lucroAtual = {'codigo':codigo, 'data':ordem['data'], 'qnt':ordem['qnt'], 'precoMedioCarteira':self.carteira.at[codigo,'precoMedio'],
				'precoMedioVenda':ordem['precoMedio'], 'lucroMedio':lucroMedio, 'lucro':ordem['qnt'] * lucroMedio}
			
			self.histLucro = self.histLucro.append(lucroAtual, ignore_index=True)
			self.lucro += ordem['qnt'] * (ordem['precoMedio'] - self.carteira.at[codigo,'precoMedio'])
			
			#Remove a quantidade vendida do total
			...
			#Se ainda está na carteira, atualiza valores
				...
				self.carteira.at[codigo,'total'] = self.carteira.at[codigo,'qnt'] * self.carteira.at[codigo,'precoMedio']
				self.carteira.at[codigo,'precoParaLucro'] = (self.carteira.at[codigo,'total'] + self.taxaMedia) / self.carteira.at[codigo,'qnt']
		#--------------------------------------
	...
	def updatePrecoParaLucro(self):
		...
		self.carteira['precoParaLucro'] = (self.carteira['total'] + self.taxaMedia) / self.carteira['qnt']
	...
	def movimentacaoMensal(self):
		...
		ordems['compras'] = self.hist.apply(lambda col: col['total'] if col['tipo'] == 'compra' else 0, axis=1)
		ordems['vendas']  = self.hist.apply(lambda col: col['total'] if col['tipo'] == 'venda'  else 0, axis=1)
	...
	def resumo(self):
		...
		print("\n\nDados:")
		print("Total comprado:      R$%10.2f" %( self.getCompras()['total'].sum() ))
		print("Total vendido:       R$%10.2f" %( self.getVendas()['total'].sum() ))
		print("")
		print("Custódia total:      R$%10.2f" %( self.carteira['total'].sum()  ))
		print("Lucro líquido total: R$%10.2f" %( self.histLucro['lucro'].sum() ))
		print("")
	
	
	
	def estimarLucro(self, codigo:str, precoPorAcao:float, qnt:int=-1, taxas:float=-1, verbose:bool=True):
		...
		#Checa se é uma venda impossível
		codigo = codigo.upper()
		assert codigo in self.carteira.index, "Código "+codigo+" não foi encontrado na carteira"
		
		if qnt > self.carteira.at[codigo,'qnt']:
			print("Erro! Foi passada uma venda de %d ações %s, mas na carteira há apenas %d dessas ações!"
				%(qnt, codigo, self.carteira.at[codigo,'qnt']))
			return
		#----------------------------
		...
		lucro = (qnt * precoPorAcao - self.taxaMedia) - (qnt * cartRow['precoMedio'])
		
		if verbose:
			print("Lucro na venda de %d ações %s à %f cada" %(qnt, codigo, precoPorAcao))
